######version 9 extract RNA-seq raw data as matrix 
######spearman correlation for certain_id list
...
####access to RNA-seq data
def RNA_count(experiment_id,status):
    ribo_object = ribo_data("%s.ribo"%experiment_id)
    RNA_tmp=ribo_object.get_rnaseq()[['CDS']]
    RNA_tmp=RNA_tmp.reset_index(level=0,drop=True)
    RNA_tmp=RNA_tmp.rename(columns={'CDS':'%s'%experiment_id})
    RNA_tmp.index.names = ['transcript']
    RNA_count_fin=RNA_tmp.add_suffix(status)
    return RNA_count_fin
            ...
            for j in v:
                try:
                    ...
                    try:
                        ...
                        RNA_count_sum=RNA_count(j,status)
                        ...
                        try:
                            ...
                            RNA_new=pd.merge(RNA_new,RNA_count_sum,on='transcript')                            
                        except KeyError:
                            new=pd.DataFrame(index=CDS_count_length_sum.index)
                            RNA_new=pd.DataFrame(index=RNA_count_sum.index)
    #display_correlation(new,outdir,status)
    return new,CDS_per,CDS_region,RNA_new
...
def run(*args):
    ...
    parser.add_argument("--outdir", help='output directory', default='/scratch/users/yliu5/ribo_cor_analysis/5_26_22/')
    try:
    ...
    print("dedup")
    ribo_dedup,dedup_CDS_percentage,dedup_CDS_region,ribo_rna_dedup=combine_ribo(args2.GSMinput,args2.ribinput,args2.outdir,status="dedup")
    dedup_per_df=pd.DataFrame(dedup_CDS_percentage.items())
    dedup_region=pd.DataFrame(dedup_CDS_region.items())
    ribo_dedup.to_csv("/scratch/users/yliu5/ribo_cor_analysis/5_26_22/cancer_ribo_dedup.csv")
    dedup_per_df.to_csv("/scratch/users/yliu5/ribo_cor_analysis/5_26_22/cancer_readpct_dedup.csv")
    dedup_region.to_csv("/scratch/users/yliu5/ribo_cor_analysis/5_26_22/cancer_readregion_dedup.csv")
    ribo_rna_dedup.to_csv("/scratch/users/yliu5/ribo_cor_analysis/5_26_22/cancer_ribo_rna_dedup.csv")
